...
export default function EmotionPage({ onEmotionDetected, onNavigate }) {
  ...
  const handleGeneratePlaylist = () => {
    if (!detectedEmotion) {
      setError('Detect an emotion first before generating a playlist.')
      return
    }

    // store for PlaylistPage fallback and pass as query param so it's explicit
    const mood = (detectedEmotion || 'neutral').toString().toLowerCase()
    try { sessionStorage.setItem('detectedMood', mood) } catch (e) { /* ignore */ }

    // navigate with query param for immediate clarity
    if (onNavigate) onNavigate('playlist')
    else navigate(`/playlist?mood=${encodeURIComponent(mood)}`)
  }
              ...
              {detectedEmotion && !loading && (
                <div className="mt-6 p-4 rounded-md border border-base-300 bg-base-200 dark:bg-base-300 dark:border-base-400">
                  ...
                  <Button
                    ...
                    className="w-full mt-4 btn btn-success border border-white border-opacity-30"
                    size="sm"
                  >
